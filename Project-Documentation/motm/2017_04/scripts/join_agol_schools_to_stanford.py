...
sd_df_ba.loc[sd_df_ba.leaid.isin(sd_not_in_times.GEOID),:].to_file("sd_not_in_nyt_ba.shp")
...
sd_ni_d = sd_df_ba.loc[sd_df_ba.leaid.isin(sd_not_in_times.GEOID),:].dissolve(by="STATEFP")

sd_not_in_nyt_boundary = sd_ni_d.ix[0].geometry

rdfn_slct = rdfn.loc[rdfn.geometry.within(sd_not_in_nyt_boundary),:]

rdfn_slct.place_name.to_csv("places_we_can_add.csv")

rdfn_slct.to_file('places_we_can_add.shp')

rdfn_slct2 = rdfn.loc[rdfn.geometry.intersects(sd_not_in_nyt_boundary),:]

rdfn_slct2.place_name.to_csv("places_we_can_maybe_add.csv")

rdfn_slct2.to_file('places_we_can_maybe_add.shp')

cnty_ba.to_file('bay_area_counties.shp')

rdfn_slct.to_file('places_we_can_add.geojson')

rdfn_slct2.place_name.to_csv("places_we_can_maybe_add.geojson")